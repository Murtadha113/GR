...
export default function SupervisorMeetings() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch incoming meeting requests for this supervisor
  ...
  const approveRequest = async (request: any) => {
    ...
    try {
      ...
      await updateDoc(doc(getFirebaseDb(), "meeting_requests", request.id), { status: "approved" })
  ...
  const rejectRequest = async (request: any) => {
    try {
      await updateDoc(doc(getFirebaseDb(), "meeting_requests", request.id), { status: "rejected" })
                ...
                >
                  ...
                  <CardContent>
                    ...
                    <div className="mt-4 flex gap-2">
                      {req.status === "pending" && (
                        <>
                          <Button onClick={() => approveRequest(req)} className="rounded-lg">
                            الموافقة
                          </Button>
                          <Button variant="destructive" onClick={() => rejectRequest(req)} className="rounded-lg">
                            رفض
                          </Button>
                        </>
                      )}
